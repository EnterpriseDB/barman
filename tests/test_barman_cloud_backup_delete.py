...
class TestCloudBackupDelete(object):
    ...
    def test_history_files_preserved_when_cleaning_up_wals(
        ...
        # THEN the cloud provider was only asked to delete the backup files and backup.info
        # for the deleted backup and the WALs which pre-date the deleted backup and *not* the
        # history file.
        self._verify_only_these_backups_deleted(
            get_cloud_interface_mock,
            backup_metadata,
    ...
    def test_no_backups_or_wals_deleted_when_dry_run_set(
        ...
        assert (
            ...
            "'wals/000000010000000000000074.gz'] due to --dry-run option"
        ) in out
    ...
    def test_wals_cleaned_up_after_deleting_by_retention_policy(
        ...
        # THEN the cloud interface was only used to delete the files associated with
        # the backups which are not required to meet the policy AND after each backup
        # was deleted, the WALs which pre-date that backup were deleted too.
        self._verify_only_these_backups_deleted(
            get_cloud_interface_mock,
            backup_metadata,
    ...
    def test_wals_on_other_timelines_are_preserved(
        ...
        backup_id = "20210723T095432"
        oldest_backup_id = "20210722T095432"
        alt_timeline_backup_id = "20210722T105432"
        begin_wals = {
        ...
        # backup.info for the deleted backup and only delete the WALs which pre-date
        # the deleted backup which are on the same timeline as the deleted backup.
        self._verify_only_these_backups_deleted(
            get_cloud_interface_mock,
            backup_metadata,
            [oldest_backup_id],
    ...
    def test_wals_are_preserved_if_older_backup_exists_on_alt_timeline(
        ...
        TODO Is this definitely the right behaviour? It feels like the right
        thing to do would be to look for previous backups on the same timeline
        rather than just any previous backup. But... is there a benefit in
        preserving the WALs on the alt timeline even after backups are deleted?
        It would allow recovery back onto that alt timeline from the original
        timeline, for example. But since the backups on the alt timeline were
        intentionally deleted, is that something we actually want to be possible?
        """
        # GIVEN a backup catalog with two backups with begin_wal values
    ...
    def test_wals_on_timelines_with_no_backups_are_deleted(
        ...
        TODO: Is this the desired behaviour? It seems to run contrary to the result
        of the previous test - that is, once backups have been deleted on a timeline
        (at which point the WALs are preserved), a subsequent deletion of the oldest
        backup is going to cause them to be deleted.
        So why did we preserve them in the first place? There still may be a valid
        recovery path from a more recent backup to the WALs on that timeline...
        """
        # GIVEN a backup catalog with two backups with begin_wal values
        backup_id = "20210723T095432"
        oldest_backup_id = "20210722T095432"
    ...
    def test_error_on_delete_wal(
        ...
        assert (
            ...
            "['wals/000000010000000000000075.gz'], Reason: Something went wrong on delete"
            in caplog.text
        )
