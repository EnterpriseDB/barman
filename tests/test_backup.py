...
class TestBackup(object):
    ...
    @pytest.fixture
    def wal_dir(self, tmpdir):
        return tmpdir.mkdir("wals")

    @pytest.fixture
    def wal_file(self, wal_dir):
        wal_file = wal_dir.join("0000000100000000/000000010000000000000001")
        wal_file.ensure()
        return wal_file

    @pytest.fixture
    def initial_backup_manager(self, tmpdir, wal_dir):
        """
        Creates a BackupManager backed by the filesystem
        """
    ...
    @pytest.fixture
    def barman_files(self, initial_backup_manager, wal_dir, tmpdir):
        # Create a fake backup directory inside tmpdir (old format)
        barman_files = {}
        barman_files["base_dir"] = tmpdir.mkdir("base")
        barman_files["backup_dir"] = barman_files["base_dir"].mkdir("fake_backup_id")
        barman_files["pg_data"] = barman_files["backup_dir"].mkdir("pgdata")
        barman_files["pg_data_v2"] = barman_files["backup_dir"].mkdir("data")
        barman_files["pre_backup_dir"] = barman_files["base_dir"].mkdir("fake_backup")
        barman_files["pre_backup_pg_data"] = barman_files["pre_backup_dir"].mkdir(
            "pgdata"
        )
        barman_files["pre_backup_pg_data_v2"] = barman_files["pre_backup_dir"].mkdir(
            "data"
        )
        barman_files["wal_history_file02"] = wal_dir.join("00000002.history")
        barman_files["wal_history_file03"] = wal_dir.join("00000003.history")
        barman_files["wal_history_file04"] = wal_dir.join("00000004.history")
        barman_files["wal_history_file02"].write(
            '1\t0/2000028\tat restore point "myrp"\n'
        )
        barman_files["wal_history_file03"].write(
            '1\t0/2000028\tat restore point "myrp"\n'
        )
        barman_files["wal_history_file04"].write(
            '1\t0/2000028\tat restore point "myrp"\n'
        )
        barman_files["wal_history_file04"].write("2\t0/3000028\tunknown\n")
        initial_backup_manager.server.config.basebackups_directory = barman_files[
            "base_dir"
        ].strpath
        initial_backup_manager.server.config.wals_directory = wal_dir.strpath
        return barman_files

    @pytest.fixture
    def b_info(self, initial_backup_manager):
        # The following tablespaces are defined in the default backup info
        # generated by build_test_backup_info
        return build_test_backup_info(
            backup_id="fake_backup_id",
            server=initial_backup_manager.server,
        )

    @pytest.fixture
    def b_pre_info(self, initial_backup_manager):
    ...
    @pytest.fixture
    def backup_manager(self, initial_backup_manager, b_info, b_pre_info):
        initial_backup_manager.get_available_backups = Mock()
        initial_backup_manager.get_available_backups.return_value = {
            "fake_backup": b_pre_info,
            "fake_backup_id": b_info,
        }
        return initial_backup_manager

    def test_delete_backup_minimum_redundancy_not_satisfied(
        self,
        backup_manager,
        wal_file,
        barman_files,
        ...
        """
        minimum redundancy not satisfied
        """
        caplog_reset(caplog)
        ...
        assert os.path.exists(barman_files["pg_data"].strpath)
        assert not os.path.exists(barman_files["pg_data_v2"].strpath)
        assert os.path.exists(wal_file.strpath)
        assert os.path.exists(barman_files["wal_history_file02"].strpath)
        assert os.path.exists(barman_files["wal_history_file03"].strpath)
        assert os.path.exists(barman_files["wal_history_file04"].strpath)

    def test_delete_backup_old_format(
        self,
        backup_manager,
        wal_file,
        barman_files,
        ...
        """
        normal delete expecting no errors (old format)
        """
        caplog_reset(caplog)
        b_info.set_attribute("backup_version", 1)
        build_backup_directories(b_info)
        ...
        assert "ERROR" not in caplog.text
        assert not os.path.exists(barman_files["pg_data"].strpath)
        assert not os.path.exists(barman_files["pg_data_v2"].strpath)
        assert os.path.exists(wal_file.strpath)
        assert os.path.exists(barman_files["wal_history_file02"].strpath)
        assert os.path.exists(barman_files["wal_history_file03"].strpath)
    ...
    def test_delete_backup_already_deleted(
        self,
        backup_manager,
        wal_file,
        barman_files,
        ...
        """
        delete the backup again, expect a failure in log
        """
        backup_manager.delete_backup(b_info)
        caplog_reset(caplog)
        backup_manager.delete_backup(b_info)
    ...
    def test_delete_backup_new_format(
        self,
        backup_manager,
        wal_file,
        barman_files,
        ...
        """
        normal delete expecting no errors (new format)
        """
        caplog_reset(caplog)
    ...
    def test_delete_first_backup(
        ...
        """
        # normal delete of first backup no errors and no skip
        ...
        assert "WARNING" not in caplog.text
        assert "ERROR" not in caplog.text
        assert not os.path.exists(barman_files["pre_backup_pg_data"].strpath)
        assert not os.path.exists(barman_files["pre_backup_pg_data_v2"].strpath)
        assert not os.path.exists(wal_file.strpath)
        assert os.path.exists(barman_files["wal_history_file02"].strpath)
        assert os.path.exists(barman_files["wal_history_file03"].strpath)
        assert os.path.exists(barman_files["wal_history_file04"].strpath)

    def test_delete_first_backup_mixed_timelines(
        self,
        backup_manager,
        wal_file,
        barman_files,
        ...
        """
        backup_manager.get_available_backups.return_value = {
            "fake_backup": b_pre_info,
            "fake_backup_after_pre": build_test_backup_info(
                backup_id="fake_backup_timeline_2",
    ...
    def test_delete_backup_with_error(
        self,
        backup_manager,
        wal_file,
        barman_files,
        ...
        """
        simulate an error deleting the the backup.
        """
        with patch(
