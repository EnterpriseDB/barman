...
class TestConsoleWriter(object):
    ...
    def test_result_show_backup_any_mode(self, capsys):
        ...
        :param _type: backup type mock parameter
    ...
    @pytest.mark.parametrize(
        ("backup_method", "parent_backup_id", "chidren_backup_ids"),
        [
            ("postgres", None, ["1234", "123456"]),
            ("postgres", 12345, ["1234", "123456"]),
            ("rsync", None, None),
        ],
    )
    def test_result_show_backup_specific_fields_by_backup_type(
        self, backup_method, parent_backup_id, chidren_backup_ids, capsys
    ):
        """
        Unit test for the show-backup command that display information in the
        console about the specific information for each type of the following
        backups: 'rsync' , 'incremental' and 'full'.
        ...
        if ext_info["mode"] == "postgres":
            ...
            if ext_info["backup_type"] == "incremental":
                for n_row in incremental:
                    ...
                    assert (
                        TestConsoleWriter.nested_row.format(n_row[0], n_row[1])
                        not in out
                    )
        assert err == ""
    ...
    def test_result_show_backup_any_mode_copy_stats(self, capsys):
        ...
        if "number_of_workers" in ext_info:
...
class TestJsonWriter(object):
    ...
    def test_result_show_backup(self, capsys):
        ...
        This unit tests checks if all fields of a backup are correctly rendered
        to the standard output as a json data structure.
        ...
        assert ext_info["cluster_size"] == json_output[server_name]["cluster_size"]
