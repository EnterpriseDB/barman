...
class CheckStrategy(object):
    ...
    def result(self, server_name, status, hint=None, check=None, perfdata=None):
        ...
        if not status:
            ...
            _logger.debug("Check '%s' succeeded for server '%s'" %
                          (check, server_name))
...
class Server(RemoteStatusMixin):
    ...
    def _init_archivers(self):
        ...
        if self.config.streaming_archiver:
            try:
                self.archivers.append(
                    StreamingWalArchiver(self.backup_manager))
    ...
    def check_archive(self, check_strategy):
        ...
        self._check_wal_queue(check_strategy, "streaming",
                              "streaming_archiver")
    ...
    def check_wal_validity(self, check_strategy):
        ...
        if self.config.last_wal_maximum_age is not None:
            ...
            # format the output
            check_strategy.result(
                self.config.name, wal_age_isok, hint=wal_message)
        else:
    ...
    def list_backups(self):
        ...
        for key in sorted(backups.keys(), reverse=True):
            ...
            output.result("list_backup", backup,
    ...
    def get_required_xlog_files(
        ...
        if target_tli and type(target_tli) is str:
            if target_tli == "current":
                ...
                raise ValueError(
                    "%s is not a valid timeline keyword" % target_tli)
    ...
    def get_wal_full_path(self, wal_name):
        ...
        hash_dir = os.path.join(
            self.config.wals_directory, xlog.hash_dir(wal_name))
    ...
    def get_wal_possible_paths(self, wal_name, partial=False):
        ...
        incoming_path = os.path.join(
            self.config.incoming_wals_directory, wal_name)
        paths.append(incoming_path)

        # Path in streaming directory
        streaming_path = os.path.join(
            self.config.streaming_wals_directory, wal_name)
    ...
    def get_wal_info(self, backup_info):
        ...
        if wal_info["wal_first"] and wal_info["wal_last"]:
            ...
            wal_info["wal_total_seconds"] = wal_last_timestamp - \
                wal_first_timestamp
    ...
    def get_wal_sendfile(self, wal_file, compression, destination):
        ...
        wal_info = self.backup_manager.compression_manager.get_wal_file_info(
            wal_file)
    ...
    def sync_cron(self, keep_descriptors):
        ...
        try:
            ...
            output.error(
                "Failed to retrieve the primary node status: %s" % force_str(
                    exc)
            )
            return
        ...
        if remote_info["backups"]:
            ...
            sync_backup_list = set(
                remote_info["backups"]) - set(local_backup_list)
        else:
        ...
        for backup_id in sorted(sync_backup_list):
            ...
            try:
                ...
                self.check_sync_required(
                    backup_id, remote_info, local_backup_info)
    ...
    def check_sync_required(self, backup_name, primary_info, local_backup_info):
        ...
        * if it exists and is FAILED locally: raise SyncToBeDeleted (remove it)
        ...
        if backup_name not in backups and local_backup_info is None:
            raise SyncError(
                "Backup %s is absent on %s server" % (
    ...
    def sync_backup(self, backup_name):
        ...
        try:
            ...
            self.check_sync_required(
                backup_name, primary_info, local_backup_info)
        except SyncError as e:
        ...
        try:
            with ServerWalSyncLock(
                ...
                try:
                    ...
                    first_backup = (
                        self.get_backup(
                            first_backup_id) if first_backup_id else None
                    )
    ...
    def write_sync_wals_info_file(self, primary_info):
        ...
        try:
            with open(
                os.path.join(self.config.wals_directory,
                             SYNC_WALS_INFO_FILE), "w"
            ) as syncfile:
                syncfile.write(
                    "%s\t%s"
