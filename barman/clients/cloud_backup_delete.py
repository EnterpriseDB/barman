...
def _get_files_for_backup(catalog, backup_info):
    ...
    for oid, backup_file in sorted(
        ...
        for file_info in [backup_file] + sorted(
            ...
            if backup_file.path is not None:
                logging.debug(
...
def _remove_wals_for_backup(cloud_interface, catalog, deleted_backup, dry_run):
    ...
    if not previous_backup:
        # We can only safely remove up to the next remaining backup if we
        # know exclusive backups are always used. Because we are barman-cloud
        # we cannot know this is the case so must assume concurrent backups.
        # In this case it is only safe to remove WALs up to the backup being
        # deleted.
        # See comments in barman.backup.BackupManager.delete_backup.
        remove_until = deleted_backup
        # A WAL is only a candidate if it is on the same timeline as the backup
        # so use BackupManager to get a set of all other timelines which have
        # backups, so that we can avoid removing WALs on other timelines.
