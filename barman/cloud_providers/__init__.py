...
def get_snapshot_interface_from_backup_info(backup_info, config=None):
    ...
    if backup_info.snapshots_info.provider == "gcp":
        ...
        if config is not None:
            ...
            aws_irsa = getattr(config, "aws_irsa", False)
            profile = getattr(config, "aws_profile", None)

            if not aws_irsa and not profile:
                raise SystemExit(
                    "Unable to locate credentials. You should configure an AWS IAM "
                    "role or an AWS profile."
                )
            if not aws_irsa:
                _logger.warning("Unable to locate IAM role. Trying AWS profile.")
